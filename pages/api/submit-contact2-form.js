...
const collectionName = "contact2_submissions"; // Changed collection name
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        console.log('Received form data for contact2:', formData); // Added logging for received data

        // --- 1. Save to MongoDB --- 
        const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
        try {
            await client.connect();
            console.log('Connected to MongoDB for contact2'); // Added logging
            const database = client.db(dbName);
            const collection = database.collection(collectionName);
            const result = await collection.insertOne(formData);
            console.log('MongoDB insertion result for contact2:', result);
        } catch (error) {
            console.error('Error saving to MongoDB for contact2:', error);
            return res.status(500).json({ success: false, message: 'Error saving to database', error: error.message }); // Added error message
        } finally {
            await client.close();
            console.log('Disconnected from MongoDB for contact2'); // Added logging
        }
        ...
        try {
            ...
            console.log('Email sent for contact2: %s', info.messageId);
            console.log('Preview URL for contact2: %s', nodemailer.getTestMessageUrl(info));

            res.status(200).json({ success: true, message: 'Form submitted successfully!', previewURL: nodemailer.getTestMessageUrl(info) });
        } catch (error) {
            console.error('Error sending email for contact2:', error);
